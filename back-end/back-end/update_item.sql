...
BEGIN
    ...
    IF p_new_quantity IS NULL OR p_new_quantity < 0 THEN
        ...
        RETURN;
    END IF;
    IF p_new_low_stock_threshold IS NOT NULL AND p_new_low_stock_threshold < 0 THEN
        o_error_message := 'Noul prag de stoc minim nu poate fi negativ.';
        RETURN;
    END IF;
    ...
    IF o_rows_updated = 1 THEN
        COMMIT;
        o_error_message := 'Articolul a fost actualizat cu succes.';
    ELSIF o_rows_updated = 0 AND o_error_message IS NULL THEN
        o_error_message := 'Articolul nu a fost gasit pentru actualizare sau nu apartine utilizatorului curent (dupa verificare).';
        ROLLBACK;
    END IF;
