...
BEGIN
    ...
    SELECT COUNT(*) INTO v_item_exists FROM items WHERE item_id = p_item_id AND user_id = p_user_id;
    IF v_item_exists = 0 THEN
        o_error_message := 'Articolul specificat nu exista sau nu aveti permisiunea.';
        RETURN;
    END IF;

    SELECT COUNT(*) INTO v_storage_belongs_to_user FROM storages WHERE storage_id = p_storage_id AND user_id = p_user_id;
    IF v_storage_belongs_to_user = 0 THEN
        o_error_message := 'Noul depozit specificat nu exista sau nu va apartine.';
        RETURN;
    END IF;

    IF p_category_id IS NOT NULL THEN
        SELECT COUNT(*) INTO v_category_belongs_to_user FROM categories WHERE category_id = p_category_id AND user_id = p_user_id;
        IF v_category_belongs_to_user = 0 THEN
            o_error_message := 'Noua categorie specificata nu exista sau nu va apartine.';
            RETURN;
        END IF;
    END IF;
...
EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        o_rows_updated := 0;
        o_error_message := 'Eroare SQL: ' || SQLERRM;
END update_item;
/